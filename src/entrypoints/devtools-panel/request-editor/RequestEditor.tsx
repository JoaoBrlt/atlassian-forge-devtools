...
import { buildEditableRequest, buildEditableHeader } from "@/utils/request-utils";
...
function RequestEditor({ request, isSending, onSend, onClose }: RequestEditorProps) {
  ...
  const handleHeaderAdd = () => {
    setEditedRequest((previous) => ({ ...previous, headers: [...previous.headers, buildEditableHeader()] }));
  };
  ...
  return (
    <div className="flex h-full w-full min-w-[320px] flex-col gap-0 overflow-hidden">
      ...
      <div className="flex h-full w-full flex-col gap-0 overflow-auto">
        <Accordion multiple defaultValue={["general", "headers", "body"]} className="flex-1">
          ...
          <AccordionItem
            value="body"
            className="flex flex-1 flex-col border-none *:data-[slot=accordion-content]:flex-1"
          >
            <AccordionTrigger className="cursor-pointer rounded-none border-0 border-y border-border bg-muted p-1.5 text-xs hover:no-underline">
              {editedRequest.type === "invokeRemote" ? "Body" : "Payload"}
            </AccordionTrigger>
            <AccordionContent className="h-full text-xs">
              <div className="h-full min-h-64 w-full overflow-hidden">
                <Suspense fallback={<JsonEditorSkeleton />}>
                  <JsonEditor prettify={false} data={editedRequest.body} onChange={handleBodyChange} />
                </Suspense>
              </div>
            </AccordionContent>
          </AccordionItem>
